...
async function callAssistant(systemPrompt, userMessage) {
  if (!AI_API_KEY || AI_API_KEY === 'your_anthropic_api_key_here') {
    // No live external API configured, use local smart fallback logic instead
    return null;
  }
...
If information is missing, make reasonable inferences or leave as empty string.`;
  ...
  const aiText = await callAssistant(system, text);
...
Suggest 8-12 skills. Exclude skills already in current_skills.`;
  ...
  const aiText = await callAssistant(system, `Role: ${role}. Current skills: ${current_skills.join(', ')}`);
...
Target roles: ${desired_roles?.join(', ')}`;
  ...
  const aiText = await callAssistant(system, context);
...
ALWAYS respond with JSON in this format:
{
  ...
  const aiText = await callAssistant(system, message);
...
Match should be 0-100.`;
  ...
  const aiText = await callAssistant(system, `Skills: ${skills?.map(s=>s.name).join(', ')}. Experience: ${experience?.map(e=>e.role).join(', ')}`);
